#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""llm_host.py — 宿主：DeepSeek 引擎 + 六组件骨架 + llm-dev 插件装载

与 rust/kernel 版同骨架（engines/hooks/governance 完全复用），换 LLM 领域工具表：
验证金字塔 L1 lint → L2 单测 → L3 生成冒烟 → L4 最小评测。

零依赖自检: python3 llm_host.py --self-test
真实运行:   python3 llm_host.py --task "..." （需 pip install openai + KH_API_KEY）
"""
import argparse
import json
import os
...
TOOLS = [
    _tool("read_file", "读文件（限 LLM_PROJECT/插件目录）",
          {"path": {"type": "string"}, "offset": {"type": "integer", "default": 1}}, ["path"]),
    _tool("grep_tree", "LLM_PROJECT 内 grep -rn（找算法实现/超参/seed 用法）",
    ...
    _tool("run_verify", "跑命令。exit code 即证据。pip install / wandb 上传会被拦",
          {"cmd": {"type": "string"}}, ["cmd"]),
    _tool("write_file", "写文件（白名单：LLM_PROJECT + state/）",
          {"path": {"type": "string"}, "content": {"type": "string"}}, ["path", "content"]),
    _tool("llm_lint", "L1: py_compile 全部 .py + ruff（若装了）",
          {"target": {"type": "string", "description": "文件或目录"}}, []),
    _tool("llm_test", "L2: pytest（缺则 unittest discover）。测试不过=没完成",
          {"target": {"type": "string", "description": "测试路径/节点，缺省全跑"}}, []),
    _tool("llm_smoke", "L3: toy 生成冒烟：模型加载+短生成+PPL 自检（三级降级，诚实标注）",
          {}, []),
    _tool("llm_eval", "L4: L4: 最小评测（PPL+生成健全性），防模型完全坏掉的回归",
...
def exec_tool(name, args):
    ...
    if name == "llm_lint":
        return _sh(f"bash tools/llm_lint.sh {args.get('target', rp or '.')}")
    if name == "llm_test":
        return _sh(f"bash tools/llm_test.sh {args.get('target', '')}")
    if name == "llm_smoke":
        return _sh("python3 tools/llm_smoke.py")
    if name == "llm_eval":
        return _sh(f"python3 tools/llm_eval.py --texts {args['cmd']}")
    if name == "graph_guard":
...
def deep_plan(question):
    ...
    sys_p = (Path(ROOT / "AGENTS.md").read_text()[:6000]
             + "\n\n你是 LLM 研究审查员。给出：改动文件/验证层级/风险点（tokenizer?chat template?评测泄漏?）。不写代码。")
    d = resolve_dialect()
    try:
...
def run(task, max_turns=MAX_TURNS_DEFAULT):
    ...
    ledger = Ledger()
    agents_md = (ROOT / "AGENTS.md").read_text() if (ROOT / "AGENTS.md").exists() else "You are an LLM research agent."
...
def self_test():
    ...
    # L3 冒烟真实跑（纯标准库，秒级）
    rc = subprocess.run("python3 tools/llm_smoke.py", shell=True, cwd=str(ROOT),
                        capture_output=True, text=True, timeout=120)
    check("llm_smoke.py 训练冒烟", rc.returncode == 0)

...
## 是什么
`rl_agent.py`：讲透Agent × 讲透RL × 讲透Prompt 的可跑缝合器（716 行纯标准库）。定位见 ../README.md——**不是生产级 RL 框架，是活教材**。

## 在此目录工作的规则
1. **改代码前先跑** `python3 rl_agent.py demo`（基线 4.7s，战报 5/6——含 1 个设计内必失败任务）；改完必须重跑对比。

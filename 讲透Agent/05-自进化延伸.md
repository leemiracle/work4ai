...
> 续篇：**[自进化2.0-整体叠加](./自进化2.0-整体叠加.md)**（2026-08-24）——五层扩成八层谱系（Skills/Harness 独立成层）+ RSI 递归自我改进四系统（STOP→DGM→DGM-H→AlphaEvolve，一级证据全核实）+ 最小 DGM 实验（[experiments/05_selfevolve.py](./experiments/05_selfevolve.py)，30 种子真跑：贪心 0/30 vs 存档 15/30，p<0.0001）。
...
| **DGM (Darwin Gödel Machine)** | [2505.22954](https://arxiv.org/abs/2505.22954) ✅已核实 | 架构+代码 | agent 自改自身代码，存档+实证评估保进化不退化；SWE-bench 20%→50%（80 迭代） | RSI 方向最强公开实证 → §4.5 |

### 4.5 整体叠加：RSI 专节（2026-08-24 新增，详见 [自进化2.0](./自进化2.0-整体叠加.md)）

当进化对象从某一层变成**整个系统（含改进机制本身）**，进入递归自我改进（RSI）：

- **STOP**（CoLLoN 2024）：种子改进器应用于自身代码——脚手架递归自改首秀。
- **DGM**（[2505.22954](https://arxiv.org/abs/2505.22954)）：自改自身 Python 代码 + 编码基准实证评估 + **变体存档保垫脚石**；SWE-bench 20%→50%。消融铁律：去掉自改进**或**去掉存档，爬坡都消失。
- **DGM-H**（[2603.19461](https://arxiv.org/abs/2603.19461)）：task agent + meta agent 合成一个可编辑程序——"改自己的改法"；四域验证超 DGM，meta 改进跨域迁移、跨 run 累积。
- **AlphaEvolve**（[2506.13131](https://arxiv.org/abs/2506.13131)）：生产化 RSI——4×4 复矩阵乘 48 次乘法（56 年首超 Strassen）、Gemini 内核提速 23%、2026-05 起为 Google TPU/基础设施常规组件。

**四判据**（[2606.09663](https://arxiv.org/abs/2606.09663)）：可检视目标系统 / 元级修改器 / 反馈导向选择 / 递归延续。**校准**：这是工程可行性证据，不是智能爆炸证据——基础模型冻结、基准人类写、外层循环手工。

**本地实验复刻**：[experiments/05_selfevolve.py](./experiments/05_selfevolve.py)（贪心 0/30 焊死局部峰 vs 存档+归因 15/30 登顶，双消融显著；核心发现：**探索权 > 探索技巧**——给垫脚石留继承权比更会变异更重要）。
...
自进化是 2025-2026 Agent 研究的主轴（PaperAgent §十 Agentic RL 的六大能力里 Self-Improvement 即其一）。横向：与 [讲透学习型Agent/](./讲透学习型Agent/README.md)（Model Collapse 边界）、[讲透记忆/](./讲透记忆/)（What-记忆层展开）、harness 镜（验证即证据=进化稳定器）三点互锁。

---
立项：2026-08-17 · 一级论文 arXiv 全核实（2401.10020/2504.20073/2507.03616 经 abs+综述交叉；2303.11366/2408.08435/2410.10762 双源；DGM 2505.22954 于 2026-08-24 补核 ✅）
升级：2026-08-24 · 新增 §4.5 RSI 专节 + 续篇卡 [自进化2.0-整体叠加](./自进化2.0-整体叠加.md)（八层谱系/DGM-H 2603.19461/AlphaEvolve 2606.13131/四判据 2606.09663/最小 DGM 实验）

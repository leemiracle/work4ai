...
> 三部曲升维：`../../讲透Context/`（窗口里放什么）→ `../../讲透Loop/`（跨 run 怎么转——2026-06 命名新学科，07-ReAct 是其内循环的 prompt 侧起源）。
...
| **09** | [`09-Prompt自动优化.md`](09-Prompt自动优化.md) | **APO：从手搓到搜索**——优化形式化（三空间四范式）/AWS 5-part/方法谱系速查表（APE/ProTeGi/DSPy 等 9 法）/决策树/AlignPro 上界 | `experiments/09_apo.py` ✅ 文本梯度 93.4% vs 随机 70.5% |
| **实证** | [`Prompt-SE实证线-软件工程.md`](Prompt-SE实证线-软件工程.md) | **证据说什么**——14 技术×10 SE 任务×4 模型（2506.05614 无万能技术/ES-KNN 最稳/o3-mini 异常/省 8k token）+ Prompt-SE 2026 两篇详解 + 可复现性危机（TMLR 2025）与"隐藏 CoT"真相 + SE 决策速查表 | — |

...
| 方法 | 机制一句话 | 何时用 | 成本 |
|---|---|---|---|
| **APE** [2211.01910](https://arxiv.org/abs/2211.01910) (2022) | 从几百个输入输出对"反向诱导"指令 | 完全没思路时生成种子 | 低（几十次调用）|
| **ProTeGi** [2305.03495](https://arxiv.org/abs/2305.03495) (EMNLP 2023) | 文本梯度：错误样本→LLM 反馈→定向改写 + Beam Search | 有种子有验证集，通用默认 | 中 |
| **OPRO** [2309.03409](https://arxiv.org/abs/2309.03409) (ICLR 2024, Google DeepMind) | meta-prompt 里放"(解法, 分数)"轨迹历史，LLM 自我改进；比人写 prompt 高 8%（GSM8K）/50%（BBH）| 无代码集成，纯对话式 | 中 |
...
| **TextGrad** [2406.07496](https://arxiv.org/abs/2406.07496) (NeurIPS 2024, Stanford) | 把"梯度"泛化成任意文本反馈，自动微分整个管线（PyTorch 语法）；GPT-4o GPQA 零样本 51%→55% | 多组件管线整体优化 | 中高 |
| **DSPy** [2310.03714](https://arxiv.org/abs/2310.03714) (ICLR 2024, Stanford) | 声明式编程：签名+模块+编译器 | 工程落地首选 | 视优化器 |
| ├─ BootstrapFewShot (2023) | DSPy 入门优化器：自举生成 few-shot 示例 | 快速上手 | 低 |
| └─ **MIPRO** [2406.11695](https://arxiv.org/abs/2406.11695) (EMNLP 2024) | 贝叶斯代理模型联合搜索指令+示例（mini-batch 解多级信用分配）| 多模块管线联合编译，效果最强 | 中高 |
...
1. **评估函数是真正的瓶颈**：优化器只会朝你给的指标走。指标设计错（比如只用 accuracy 忽略安全），APO 高效地优化出错误目标——垃圾进垃圾出的加速版。实证警示见 [Prompt-SE实证线 §四](./Prompt-SE实证线-软件工程.md)：连"XX 技术有效"本身都大面积无法复现，你的 metric 更要经得起统计检验。

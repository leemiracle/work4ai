...
| **讲透DeepSeek-Harness** | DeepSeek 开源 Agent Harness（dsh）全量知识库：DeepWiki 39/39 页 + 中文知识图谱（3368 节点/5160 边/12 层架构/16 步导览，commit 76fda729）+ 新人指南 + 14 篇文件精讲（everything-is-a-plugin/Cordis/landlock 沙箱/llm 包族）⭐ 2026-09-04 建成（pipeline-bin 第三代流水线） |
| **DeepSeek 全家桶 35 仓** | [全家桶总索引](./DeepSeek全家桶-总索引.md)：deepseek-ai 全部 35 仓流水线——DeepWiki 35/35 全量（~800 页零遗漏）+ 中文知识图谱 28 仓（~1.4 万节点）+ 论文精读 16 仓（7 处 arXiv 勘误钉版）+ 基础设施 onboard/explain；系统线（3FS/DeepGEMM/FlashMLA/DeepEP/DualPipe/EPLB/LPLB/smallpond/TileKernels）+ 模型演进线（LLM→V2→V3→R1→V3.2→Engram）⭐ 2026-09-05 建成 |
| **讲透优化** | 三卷十部 52 篇：哲学（本体论/权衡统一论）+ 数学（凸/对偶/梯度/流形/鲁棒/组合）+ ML（SGD/自适应/泛化界面）+ 分布式联邦 + 系统工程桥 + 智能伦理（公平/对抗/LLM 优化 agent）+ **应用层六战**（并发/内存/IO/缓存/降级/自适应）⭐ 2026-09-02 建成 |
| **讲透图** | 图宇宙五部：数学之图（图论/谱/子式/Tutte）→ 网络科学（小世界/无标度/社群）→ 学习之图（GNN/WL 表达力/生成）→ 状态之图（自动机/马尔可夫/Petri/模型检测）→ 应用之图（分子/芯片/推荐）⭐ 2026-09-02 建成 |
...
| **讲透AI产品经理** 🆕 | [README](./讲透AI产品经理/README.md) + 00-09 十章（角色→形态→方法论→eval→概率设计→画像→社科八镜头→商业→能力模型→前沿）+ **3 个实验**（RICE 敏感性翻转/Kano 漂移/门槛经济学三段切换）+ **ai-pm agent**（本地配置 `.claude/agents/ai-pm.md`，不入公开仓）⭐ 2026-09-04 挂网 |

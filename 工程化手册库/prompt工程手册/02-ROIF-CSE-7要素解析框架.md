...
- **数量按模型分档**（2026-08-17 一手修订，推翻旧"3-5 个最佳"单一口径）：
  - **旗舰思考模型**（GLM-5+/o1/R1 类，thinking 默认开）：**0-3 个，宁少勿多**——实测 [GLM-CtxAPO](../../讲透Agent/实战案例-RL领域Agent/GLM-CtxAPO实验报告.md)：在 16/16 的最优 prompt 上仅加 1 条 few-shot，知识题被例子带偏（5/6→4/6）且思考 token ×2.4（CoT 已内化，外显例子干扰推理路径）
  - **非思考模型 / 小模型 / 纯格式对齐任务**：3-5 个（少于 3 不够，多于 5 过拟合）

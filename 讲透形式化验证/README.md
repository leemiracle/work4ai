...
> **系列宪法**：形式化验证是"**在可判定性边界的这一侧，为每类系统性质找到可计算的检查方法**"的工程学科。卷零讲为什么形式化（Lean4/神经符号视角），卷一攻 NP/coNP（把断言喂给求解器），卷二攻 PSPACE（让所有路径说话），卷三攻 P（概率定量），卷四参观引擎室（BDD/进程代数/自动机学习），收尾把 19 个工具归位到复杂度动物园的格子里——**每章末尾标注复杂度格，是全系列的横向主线**。番外两章横切：15 以 CMU 15-414 的教学顺序补**证明论段**（语义/循环归纳/WP 谓词变换子/序列演算，全系列缺口），16 以状态图（XState/奖励机器）补**形式模型的工程端点**（agent 编排 guard=guardrail + HPC 负结果判定）。
>
...
| 04 | [有界模型检查：CBMC/ESBMC](./04-有界模型检查CBMC.md) | ✅ | 程序 k 步展开→SSA→断言取反→SMT，**反例=模型**；**3 行循环程序 k=3 展开编码手推**；k-induction 突破有界；CBMC 的 goto-IR/指针数组编码 vs ESBMC 的并发上下文界/多后端；不可判定问题的"有界"出路。【NP】 | cbmc.exe（Windows 官方包，可选）+ z3 复现编码 |
| 05 | [抽象解释与 CPAchecker](./05-抽象解释与CPAchecker.md) | ✅ | 过近似：**符号函数的区间抽象第 3 轮加宽第 4 轮不动点手推**；可靠≠完备（假报警）；CEGAR 闭环（反例→Craig 插值→谓词精化，**完整走一轮**）；CPA 可配置框架=域×迁移×合并；SV-COMP 赛场格局。【不可判定→可靠近似】 | 手写区间域解释器 + CPAchecker（Java，可选） |
...
### 番外 · 课程地图与可执行形式模型（15-16）

| # | 标题 | 状态 | 核心锚点 | 仪器 |
|---|------|------|----------|------|
| 15 | [漏洞捕捉：像 CMU 15-414 那样抓 bug](./15-漏洞捕捉CMU15414.md) | ✅ | 26 讲→系列全地图；契约/语义/循环归纳/**WP 四规则**/证明论一瞥（全系列证明论缺口）；变式与终止性；mini-BMC 反例=模型；二分查找溢出二十年叙事。【契约检查不可判定；有界反例 NP】 | lab15（纯 Python：闭包式 WP + 不变式三条 VC + 有界反例） |
| 16 | [状态图与 XState：Agent/RL/AI 的可执行形式模型](./16-状态图与XState.md) | ✅ | Harel 1987→SCXML→XState v5；**guard=guardrail**（连 omega-guard）；状态图/LangGraph/Petri 网形态学三分；奖励机器（Icarte 系）乘积 MDP；策略蒸馏连 L*；HPC 负结果判定。【显式 FSM 验证=P】 | lab16（纯 Python：mini 状态图引擎 + RM Q-learning 对拍） |

### 收尾 · 复杂度动物园（50）

| # | 标题 | 状态 | 核心锚点 | 仪器 |
|---|------|------|----------|------|
| **50** | [收尾：复杂度动物园与选型决策树](./50-复杂度动物园与选型.md) | ✅ | 全系列复杂度地图（P→NP/coNP→NP∩coNP→PSPACE→不可判定，每工具归位）；**两问定位决策树**（你有什么×你要什么→工具）；工具依赖生态图（CUDD→PRISM/Storm 符号引擎；Z3←Dafny/Alloy；CBMC 自带 SAT 求解器谱系）；验证剧场批判（连卷零 00）；识别"我的问题在哪一格"三步法（判定化→证书结构→归约）。 | 决策树 + 生态图 |
...
- **想搞懂求解器怎么吞掉断言**：[03 篇](./03-SMT求解与Z3cvc5.md) → [04 篇](./04-有界模型检查CBMC.md)（DPLL(T)/CDCL 手推 + k 展开编码手推）
- **想验证 C 代码**：[04 篇](./04-有界模型检查CBMC.md)（BMC 反例=模型）→ [05 篇](./05-抽象解释与CPAchecker.md)（区间抽象 + CEGAR 精化）
- **想写规格并带上证明**：[06 篇](./06-Dafny验证语言.md)（契约/不变式/WP）→ [08 篇](./08-TLA+与TLC.md)（TLA+ 规约 + TLC 检查）
- **要验证概率系统**：[10 篇](./10-PRISM概率模型检测.md) → [11 篇](./11-Storm与高性能概率检查.md)（PCTL 定量保证 + 稀疏引擎上量）
- **想从黑盒逆向重建模型**：[14 篇](./14-自动机学习Lstar.md)（Angluin L* 观察表手推）
- **只想选个工具**：[50 篇](./50-复杂度动物园与选型.md)（两问定位决策树 + 全系列复杂度地图）
...
- **想按名校课程的顺序系统学**：[15 篇](./15-漏洞捕捉CMU15414.md)（CMU 15-414 全地图 + WP/不变式手推 + 反例=模型）
- **想把状态机用进 agent 编排 / RL 奖励**：[16 篇](./16-状态图与XState.md)（guard=guardrail + 奖励机器 + HPC 负结果）

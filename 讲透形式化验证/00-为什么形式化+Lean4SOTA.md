...
- **F2 卡壳点**：我长期把"形式化验证"和"模糊测试 + 100% 覆盖率"混为一谈，觉得都是"测得多就保证多"。重读 §1.1 表格才意识到二者**性质不同**：测试只能"证明 bug 存在"（找到一个反例），永远不能"证明 bug 不存在"；形式化验证的产出是**对所有输入的数学定理**——穷尽整个输入空间，不是抽样。Dijkstra 那句"测试只能证明 bug 存在"我之前只是字面理解，看到 seL4 那段 `theorem ... ∀ s, ...` 才明白"定理 vs 测试报告"是两种不同的认识论产物。
- **F3 术语翻译**：
  - "refinement（精化证明）" → 从最抽象的"应该做什么"到最具体的"机器码"，每一层都证明"下一层忠实执行了上一层"，像四层接力棒，每棒都公证过
  - "proof-to-code 比" → 每写 1 行被验证的程序要配多少行证明，比例越低说明验证越便宜（seL4 是 20:1，Atmosphere 把它压到 3.32:1）
  - "Curry-Howard 同构" → 在 Lean4 里"写程序"和"写证明"是同一件事——类型就是命题，函数就是证明，相当于"造出这个类型的对象 = 给出这个命题的证明"
- **F4 回炉**：v1 我把 §六"验证剧场"写成"Lean4 有 bug 所以不可信"——这是**结论错位**。v2 改成"**验证边界之外的部分静默失败**才是真陷阱：Lean4 内核漏洞 #14576 和 libcrux 的 `lax` 属性都不是'Lean 拒绝了错的证明'，而是'绕过了验证本身'。问题不在 Lean，在'以为 Lean 接受了就等于证明了'这个误区"。diff：从"工具不可信"改为"信任模型的边界感缺失"，对应 §6.3 的 CI 加固 checklist。

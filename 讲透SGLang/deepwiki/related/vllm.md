> 来源: [https://deepwiki.com/vllm-project/vllm](https://deepwiki.com/vllm-project/vllm)
> 关联理由: 同类竞品（深度资料见 work4ai/讲透vLLM）
...
|---|---|---|
| External Interface | Entry points for users | LLM, AsyncLLM, OpenAI API server vllm/entrypoints/llm.py67 vllm/v1/engine/async_llm.py72 |
| Configuration | Argument parsing and config assembly | EngineArgs, VllmConfig, ModelConfig, ParallelConfig vllm/engine/arg_utils.py35 vllm/config/vllm.py124 |
| Engine Orchestration | Request lifecycle and IPC coordination | EngineCore, InputProcessor vllm/v1/engine/core.py104 vllm/v1/engine/input_processor.py20 |
| Scheduling & Memory | Resource allocation and KV management | Scheduler, KVCacheManager, BlockPool vllm/v1/core/sched/scheduler.py73 vllm/v1/core/kv_cache_manager.py25 |
| Execution | Model forward passes on hardware | Executor, GPUModelRunner, Sampler vllm/v1/worker/gpu_model_runner.py280 vllm/v1/sample/sampler.py25 |
...
### Model Registry and Support
 ...
 - **Native vLLM Models**: Highly optimized implementations using custom kernels (e.g., LlamaForCausalLM, DeepseekV3ForCausalLM) [vllm/model_executor/models/registry.py92-153](https://github.com/vllm-project/vllm/blob/185cad

...
#ifdef USE_ROCM
__global__ void dequantize_weights(
    int* __restrict__ B,                 // [R, N]
    half* __restrict__ scaling_factors,  // [R // G, N * 8]
    int* __restrict__ zeros,             // [R // G, N]
    half* __restrict__ C,                // [R, N * 8]
    const int R, const int N, const int G, const int blockDim_x,
    const int blockDim_y, const int gridDim_x) {
  static constexpr uint32_t ZERO = 0x0;

  int col = blockIdx.x * blockDim_x + threadIdx.x;
  const int row = blockIdx.y * blockDim_y + threadIdx.y;
  const int col_stride = blockDim_x * gridDim_x;

  if (row < R) {
    for (; col < N; col += col_stride) {
      const int indexB = col + row * N;
      int* B_ptr2 = B + indexB;

      // indexB * 8; one input is dequantized to 8 outputs
      const int indexC = indexB << 3;
      half* C_ptr2 = C + indexC;

      const int indexZ = col + (int)(row / G) * N;
      int* zeros_ptr2 = zeros + indexZ;

      // indexZ * 8; one zero corresponding to 8 scales
      const int indexS = indexZ << 3;
      half* scaling_factors_ptr2 = scaling_factors + indexS;

      uint32_t zeros_loaded = *(uint32_t*)(zeros_ptr2);
      // i4{0, 1, 2, 3, 4, 5, 6, 7} => fp16{0, 4, 1, 5, 2, 6, 3, 7}
      uint4 B_loaded_zero_neg = dequantize_s4_to_fp16x2_neg(zeros_loaded);
      ...
      *reinterpret_cast<uint4*>(C_ptr2) = B_loaded_fp16;
    }
  }
}
#else
...
torch::Tensor awq_dequantize(torch::Tensor _kernel,
  ...
  if (thx == 0 && thy == 0) {
#ifdef USE_ROCM
    int pow_2 = next_pow_2(qout_c);
    constexpr int WARP_SIZE = 64;
    constexpr int MAX_THREADS = 1024;
    constexpr int MIN_THREADS = 256;
    x_thread = std::min(std::max(pow_2, WARP_SIZE), MAX_THREADS);
    y_thread = std::max(MIN_THREADS / x_thread, 1);
    x_blocks = std::max(qout_c / x_thread, 1);
    y_blocks = (in_c + y_thread - 1) / y_thread;
#else
    x_thread = 8;
    y_thread = 8;
  ...
  vllm::awq::dequantize_weights<<<num_blocks, threads_per_block, 0, stream>>>(
      kernel, scaling_factors, zeros, de_kernel, in_c /* R */, qout_c /* N */,
      G, x_thread /* blockDim_x */, y_thread /* blockDim_y */,
      x_blocks /* gridDim_x */);
#else
  vllm::awq::dequantize_weights<<<num_blocks, threads_per_block, 0, stream>>>(

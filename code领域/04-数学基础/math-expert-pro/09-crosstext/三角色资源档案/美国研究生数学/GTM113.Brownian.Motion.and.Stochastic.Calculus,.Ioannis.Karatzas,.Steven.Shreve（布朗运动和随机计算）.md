# 三角色资源档案：GTM113.Brownian.Motion.and.Stochastic.Calculus,.Ioannis.Karatzas,.Steven.Shreve（布朗运动和随机计算）

> 自动生成 · 丛书：美国研究生数学 · 主题：**分析·微积分**


## 🎭 三角色定位

| 角色 | 本书定位 |
|------|---------|
| 📘 **教材学习者** | 阶段1-2 主线 |
| 🔧 **工程师实战** | 数值分析基础 |
| 🔬 **研究员学术** | 分析方向 |

## 🌐 网络资源（按角色优先级）

### 📘 教材优先（建立直觉）

**视频课程**：
- [3Blue1Brown《微积分的本质》](https://www.3blue1brown.com/topics/calculus)
- [MIT OCW 18.01 单变量微积分](https://ocw.mit.edu/courses/18-01-single-variable-calculus-fall-2006/)
- [Khan Academy Calculus](https://www.khanacademy.org/math/calculus-1)

**经典论文/参考**：
- [Cauchy 1821 原始论文（Cours d'Analyse）](https://gallica.bnf.fr/)
- [Wigner 1960《数学的不合理有效性》](https://www.maths.ed.ac.uk/~v1ranick/papers/wigner.pdf)

### 🔧 工程师优先（实战代码）

**Python/代码实现**：
- [SymPy Calculus](https://docs.sympy.org/latest/modules/calculus/index.html)
- [Desmos 在线计算器](https://www.desmos.com/calculator)
- [GeoGebra Calculus](https://www.geogebra.org/calculus)

### 🔬 研究员优先（形式化 + 社区）

**Lean 形式化**：
- [Mathlib.Topology.Continuity](https://leanprover-community.github.io/mathlib4_docs/)
- [Mathlib.Deriv](https://leanprover-community.github.io/mathlib4_docs/)

**学术社区**：
- [Math StackExchange [calculus]](https://math.stackexchange.com/questions/tagged/calculus)
- [r/calculus](https://www.reddit.com/r/calculus/)

## 🎯 综合学习路径

### 📘 教材路径（系统建立成熟度）
1. 看 1 个 3Blue1Brown 视频建立直觉
2. 精读书中核心章节
3. 用 SymPy/numpy 跑一遍
4. 写笔记
5. 跨书对照（参考 09-crosstext）

### 🔧 工程师路径（解决具体问题）
1. 识别项目需求
2. 查书中公式/算法
3. GitHub 找开源实现
4. 真实数据验证
5. 查失败案例档案避坑

### 🔬 研究员路径（推进前沿）
1. 跨书追踪同一概念
2. arXiv 看最新论文
3. Lean 形式化
4. MathOverflow 提问
5. 写论文

## 📐 接入 math-expert-pro 13 目录

- 主题：分析·微积分
- 配套：[书籍索引](../../书籍索引/美国研究生数学/GTM113.Brownian.Motion.and.Stochastic.Calculus,.Ioannis.Karatzas,.Steven.Shreve（布朗运动和随机计算）.md)
- 配套：[书籍深度分析](../../书籍深度分析/美国研究生数学/GTM113.Brownian.Motion.and.Stochastic.Calculus,.Ioannis.Karatzas,.Steven.Shreve（布朗运动和随机计算）.md)
- 配套：主题深度报告（`../书籍索引/主题深度报告-分析·微积分.md`）
- 配套：[30 真实系统映射](../../书籍索引/475书-30真实系统-映射.md)
- 配套：[10 失败案例档案](../../失败案例档案)

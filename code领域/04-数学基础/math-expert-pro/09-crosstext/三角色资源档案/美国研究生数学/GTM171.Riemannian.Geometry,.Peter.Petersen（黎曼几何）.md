# 三角色资源档案：GTM171.Riemannian.Geometry,.Peter.Petersen（黎曼几何）

> 自动生成 · 丛书：美国研究生数学 · 主题：**几何·微分/黎曼**


## 🎭 三角色定位

| 角色 | 本书定位 |
|------|---------|
| 📘 **教材学习者** | 阶段3 几何 |
| 🔧 **工程师实战** | 广义相对论工程 |
| 🔬 **研究员学术** | 几何研究 |

## 🌐 网络资源（按角色优先级）

### 📘 教材优先（建立直觉）

**视频课程**：
- [Frederic Schuller 几何讲座](https://www.youtube.com/playlist?list=PLPH7f_7ZlzxTi6kS4vCmv4ZKm9J8_jfMw)
- [MIT OCW 18.950 微分几何](https://ocw.mit.edu/courses/18-950-differential-geometry-fall-2008/)

**经典论文/参考**：
- [Einstein 1915 广义相对论](https://einsteinpapers.press.princeton.edu/)
- [Atiyah-Singer 1963 指标定理](https://www.maths.ed.ac.uk/~v1ranick/papers/atiyahsinger.pdf)

### 🔧 工程师优先（实战代码）

**Python/代码实现**：
- [SageManifolds](https://sagemanifolds.obspm.fr/)
- [sympy.diffgeom](https://docs.sympy.org/latest/modules/diffgeom.html)

### 🔬 研究员优先（形式化 + 社区）

**Lean 形式化**：
- [Mathlib.Geometry.Manifold](https://leanprover-community.github.io/mathlib4_docs/Mathlib/Geometry/Manifold/)

**学术社区**：
- [MathOverflow [dg.differential-geometry]](https://mathoverflow.net/questions/tagged/dg.differential-geometry)

## 🎯 综合学习路径

### 📘 教材路径（系统建立成熟度）
1. 看 1 个 3Blue1Brown 视频建立直觉
2. 精读书中核心章节
3. 用 SymPy/numpy 跑一遍
4. 写笔记
5. 跨书对照（参考 09-crosstext）

### 🔧 工程师路径（解决具体问题）
1. 识别项目需求
2. 查书中公式/算法
3. GitHub 找开源实现
4. 真实数据验证
5. 查失败案例档案避坑

### 🔬 研究员路径（推进前沿）
1. 跨书追踪同一概念
2. arXiv 看最新论文
3. Lean 形式化
4. MathOverflow 提问
5. 写论文

## 📐 接入 math-expert-pro 13 目录

- 主题：几何·微分/黎曼
- 配套：[书籍索引](../书籍索引/美国研究生数学/GTM171.Riemannian.Geometry,.Peter.Petersen（黎曼几何）.md)
- 配套：[书籍深度分析](../书籍深度分析/美国研究生数学/GTM171.Riemannian.Geometry,.Peter.Petersen（黎曼几何）.md)
- 配套：[主题深度报告](../书籍索引/主题深度报告-几何·微分.md)
- 配套：[30 真实系统映射](../书籍索引/475书-30真实系统-映射.md)
- 配套：[10 失败案例档案](../失败案例档案/)

# 三角色资源档案：GTM132.Iteration.of.Rational.Functions.Complex.Analytic.Dynamical.Systems,.Alan.F..Beardon（有理函数的迭代：复解析动力系统）

> 自动生成 · 丛书：美国研究生数学 · 主题：**分析·复分析**


## 🎭 三角色定位

| 角色 | 本书定位 |
|------|---------|
| 📘 **教材学习者** | 阶段2 复分析 |
| 🔧 **工程师实战** | 信号处理 |
| 🔬 **研究员学术** | 复分析研究 |

## 🌐 网络资源（按角色优先级）

### 📘 教材优先（建立直觉）

**视频课程**：
- [MIT OCW 18.04 复变函数](https://ocw.mit.edu/courses/18-04-complex-variables-with-applications-fall-1999/)
- [Wesleyan 复分析](https://www.youtube.com/playlist?list=PLi01XoE8jYoi6q7z5zU5Vb5fX_KfB6X2r)

**经典论文/参考**：
- [Cauchy 1825 积分定理](https://gallica.bnf.fr/)
- [Riemann 1851 复分析博士论文](https://www.maths.ed.ac.uk/~v1ranick/papers/riemann1851.pdf)

### 🔧 工程师优先（实战代码）

**Python/代码实现**：
- [mpmath 复数](https://mpmath.org/)
- [SymPy 复分析](https://docs.sympy.org/latest/modules/functions/elementary.html)

### 🔬 研究员优先（形式化 + 社区）

**Lean 形式化**：
- [Mathlib.Analysis.Complex](https://leanprover-community.github.io/mathlib4_docs/Mathlib/Analysis/Complex/)

**学术社区**：
- [Math StackExchange [complex-analysis]](https://math.stackexchange.com/questions/tagged/complex-analysis)

## 🎯 综合学习路径

### 📘 教材路径（系统建立成熟度）
1. 看 1 个 3Blue1Brown 视频建立直觉
2. 精读书中核心章节
3. 用 SymPy/numpy 跑一遍
4. 写笔记
5. 跨书对照（参考 09-crosstext）

### 🔧 工程师路径（解决具体问题）
1. 识别项目需求
2. 查书中公式/算法
3. GitHub 找开源实现
4. 真实数据验证
5. 查失败案例档案避坑

### 🔬 研究员路径（推进前沿）
1. 跨书追踪同一概念
2. arXiv 看最新论文
3. Lean 形式化
4. MathOverflow 提问
5. 写论文

## 📐 接入 math-expert-pro 13 目录

- 主题：分析·复分析
- 配套：[书籍索引](../书籍索引/美国研究生数学/GTM132.Iteration.of.Rational.Functions.Complex.Analytic.Dynamical.Systems,.Alan.F..Beardon（有理函数的迭代：复解析动力系统）.md)
- 配套：[书籍深度分析](../书籍深度分析/美国研究生数学/GTM132.Iteration.of.Rational.Functions.Complex.Analytic.Dynamical.Systems,.Alan.F..Beardon（有理函数的迭代：复解析动力系统）.md)
- 配套：[主题深度报告](../书籍索引/主题深度报告-分析·复分析.md)
- 配套：[30 真实系统映射](../书籍索引/475书-30真实系统-映射.md)
- 配套：[10 失败案例档案](../失败案例档案/)

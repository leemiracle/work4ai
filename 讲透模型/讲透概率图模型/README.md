...
>
> 配套：[`讲透生成模型`](../讲透生成模型)（直系后代）+ [`讲透因果推断`](../../讲透Agent/讲透因果推断)（图上的因果）+ [`讲透信息论`](../../讲透数学/讲透信息论)（ELBO 的信息论根基）
...
> 图的数学/学习/状态全景见 [`讲透图`](../../讲透数学/讲透图/README.md)——本系列是「概率之图」象限；其 37 因果图大纲与 06-HMM 衔接本系列。
...
- 后代：[`讲透生成模型`](../讲透生成模型)
- 因果：[`讲透因果推断`](../../讲透Agent/讲透因果推断)（PGM + do-calculus）
- 信息论：[`讲透信息论`](../../讲透数学/讲透信息论)（ELBO 的根基）

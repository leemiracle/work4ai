...
| 06 | [06-生命周期.md](06-生命周期.md) | 模型作为可运维资产：六阶段决策门 + 退役三判据（训练侧叙事见 [`../讲透LLM/01`](../讲透LLM/01-完整生命周期.md)） | `06_lifecycle.py` ✅ |
| 07 | [07-模型的本质.md](07-模型的本质.md) | 它到底"懂"了什么？三立场 + 双向证据 + 三个可操作化框架（压缩比/自由能/干预主义） | （理论）|
...
- **背景**：知道"AI 要训练"即可。
- **跨域支**：只需中学物理/基础概率，无需编程背景（实验可只看结论数字）。

...
>
> 📚 **论文精读配套**（2026-08-14 新增）：[`06-CS336论文精读/`](./06-CS336论文精读/) —— **114 篇 referenced papers 逐篇深度解读 + 工程经验 + 8 个可运行验证实验**，按 10 大主题 12 个文件（历史根基 / Transformer 架构 / 预训练模型 / Scaling Laws / 数据工程 / 分布式训练 / 优化器 / 对齐 / 评估 / 2025-2026 前沿 / 灵魂论文三层深读）。本文档讲"课/作业/仓库"，论文精读子目录讲"每篇论文的核心贡献与可落地经验"。详见 [§二·补 论文精读导航](#二补-论文精读导航114-篇)。
>
...
> 2026-08-14 新增配套：把 `lectures` 仓库 `references.py` 收录的 **114 篇论文**（1950 Shannon → 2026 GLM-5/DeepSeek-V4）逐篇深度解读，按 9 大主题分文件存于 [`06-CS336论文精读/`](./06-CS336论文精读/)。
>
> **为什么单独立子目录**：本文档（课程全解）已经很长，114 篇论文的逐篇精读若内嵌会让文档臃肿到不可读。独立成子目录后，每个主题文件可深可浅，且能被本项目其他「讲透 X」系列交叉引用。

### 文件索引

| 子文档 | 主题 | 篇数 | CS336 对应 |
|---|---|---|---|
| [`A-历史根基.md`](./06-CS336论文精读/A-历史根基.md) | Shannon→神经LM→Attention前史 | 10 | L1, L3 |
| [`B-Transformer架构.md`](./06-CS336论文精读/B-Transformer架构.md) | 架构组件+长序列+MLA/Mamba/BLT | 18 | L3, L4 |
| [`C1-预训练先驱.md`](./06-CS336论文精读/C1-预训练先驱.md) | ELMo/BERT/GPT-2/T5/GPT-3 (2018-2022) | 10 | L3 |
| [`C2-开源模型实战.md`](./06-CS336论文精读/C2-开源模型实战.md) | LLaMA系/Mistral/DeepSeek/PaLM等 | 20 | L3, L4 |
| [`D-Scaling-Laws.md`](./06-CS336论文精读/D-Scaling-Laws.md) | Kaplan→Chinchilla→过训练 | 9 | L9, L11 ⭐ |
| [`E-数据工程.md`](./06-CS336论文精读/E-数据工程.md) | 数据集+过滤+RegMix配比 | 8 | L13, L14 |
| [`F-分布式训练.md`](./06-CS336论文精读/F-分布式训练.md) | GPipe/Megatron/ZeRO/3D并行 | 6 | L7, L8 |
| [`G-优化器.md`](./06-CS336论文精读/G-优化器.md) | AdamW→SOAP→Muon | 8 | L2 |
| [`H-对齐与后训练.md`](./06-CS336论文精读/H-对齐与后训练.md) | RLHF/DPO/GRPO/R1 | 8 | L15, L16 ⭐ |
| [`I-评估.md`](./06-CS336论文精读/I-评估.md) | MMLU 与评估方法论 | 1 | L12 |
| [`J-前沿2025-2026.md`](./06-CS336论文精读/J-前沿2025-2026.md) | OLMo/Qwen/GLM/DeepSeek最新报告 | 23 | 全程 |
| [`K-灵魂论文三层深读.md`](./06-CS336论文精读/K-灵魂论文三层深读.md) | 8 篇灵魂论文 直觉→数学→**实测** | 8 实验 | 全程 🧪 |

### 阅读策略

- **速览全局**：只读每篇标 ⭐⭐⭐ 的"灵魂论文"（共 ~20 篇，含 Transformer/Scaling Laws/Chinchilla/RLHF/DPO/GRPO/R1/LLaMA/DeepSeek-V2 MLA 等）
- **按 CS336 讲座顺序读**：每讲对应一个子文档，看完讲座 → 读对应论文精读 → 做 assignment
- **重点深读 3 个模块**：**D（Scaling Laws）、H（对齐）、B（架构）**——这是 CS336 理论与实践的灵魂
...
> **核心理念**：不只复述论文，而是提炼"**造 LLM 时能直接用的经验**"。
...
- **对应「讲透」**：[`讲透基础模型`](../讲透基础模型/) + [`讲透反向传播`](../讲透PyTorch) + [`讲透Transformer`](../讲透Transformer/) + [`讲透PyTorch`](../讲透PyTorch/)
...
| **手写 autograd** | [`讲透反向传播`](../讲透PyTorch) | A1（间接，A1 用 PyTorch autograd）|
...
> ✅ **已完成（2026-08-14）**：**114 篇论文逐篇深度解读 + 工程经验**已写入配套子目录 [`06-CS336论文精读/`](./06-CS336论文精读/)（详见 [§二·补](#二补-论文精读导航114-篇)）。覆盖 `references.py` 全部引用，按 9 大主题分 10 个文件，每篇含核心问题/方法/公式/工程经验/CS336 对应讲座。剩余建议：
...
> 1. **修改 05 文档**——把 05 的 2.1.A CS336 节改成"→ 详见 [06](06-CS336论文精读)"引用（避免重复）

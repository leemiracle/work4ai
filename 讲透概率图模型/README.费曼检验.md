# 费曼检验（⚠️ 不适用）· 讲透概率图模型（Probabilistic Graphical Models）

> 自动生成 2026-08-04 19:49 | 原文 [`讲透概率图模型/README.md`](../讲透概率图模型/README.md)
> 类型：⚪ 不适用（索引/编年/清单/访谈）
...
- 原文行数：约 201+ 行
- 抽到的 H1：讲透概率图模型（Probabilistic Graphical Models）
- H2 数量：3
- 黑名单术语出现：22 次（无需外行翻译，因为这不是教学）
- 数学公式：0 行内 / 0 块

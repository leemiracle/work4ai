...
> **调度器**：本表是静态地图；动态执行用 [MATH_LOOP_ENGINE.md](MATH_LOOP_ENGINE.md)——七阶段循环（理论→计划→实践→观察→反思→理论'→深挖）在 63 类数学森林上广度优先点亮，每次循环以五类验证信号（Lean 0 sorry / 符号恒等 / 数值阈值 / OEIS-LMFDB 命中 / 超 dummy 基线）之一收口。

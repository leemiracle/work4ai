...
**10 篇 = 8 篇主线（00-07）+ 2 篇补篇（08 学科X光五问、09 建模来源分类学），从"什么是建模"一路讲到 AI4Science 的前沿。**
...
graph LR
    ...
    H --> I["08 分支X光五问<br/>体系/创新/语言/构造/代码"]
    I --> J["09 建模来源分类学<br/>六层坐标:需求/领域/认识论/对象/理论/形态"]
```
...
| 08 | [08-分支X光五问.md](08-分支X光五问.md) | 学科X光：体系/近五年/语言/可构造/→代码？（宇宙 D5 模板补篇） | （理论）|
| 09 | [09-建模来源分类学.md](09-建模来源分类学.md) | 模型从哪来？六层来源坐标（每条带出处锚点+纠错存档+四缺口清单） | （理论）|
...
## 分支X光五问·速答表（详见 [08 章](08-分支X光五问.md)）

| 问 | 一句话答案 |
|----|-----------|
| ① 体系结构 | 翻译工厂：一条生产环（问题→假设→结构→求解→验证→迭代）× 三条产线（机理/统计/AI）+ 混合第四象限 |
| ② 近五年创新 | AI 流派当房东：AIFS 2025 业务化、GenCast/Aurora/NeuralGCM、算子学习、符号回归复兴、2024 双诺奖；痛点=极端事件外推 |
| ③ 语言特征 | 五级表达谱系（自然语言→方程→随机→图→程序）；逻辑底座=可证伪性；规范化停在接口级（量纲分析=建模的类型系统、SBML/FMI/V&V） |
| ④ 可构造与结构 | 结构可辨识性是第一道闸；刚性来自量纲/守恒律/对称性三源；UDE 是可构造带的新段；NFL=无结构先验的纯数据构造是幻觉 |
| ⑤ 问题→代码 | 五通道：建模 DSL、model=program（可微编程）、数字孪生、假设发现（SINDy/PySR）；不可机械化边界=假设选择/验证责任 |
...
- [`../讲透计算复杂度/`](../讲透计算复杂度/)：本系列 08 的姊妹决策表在那边 07 章——本系列答"问题属于哪型"，对面答"这单能不能接"。
- [`../讲透应用数学/`](../讲透应用数学/)：**上下游方法论**——本系列管"现实→数学结构"（翻译），对面管"数学结构→数值与洞见"（量纲/渐近/适定/稳定的手艺）。注：对面 README 将本系列列为"工具"之一，与 09 章 §0.3 的管道定位存在措辞张力，待公告栏仲裁。

...
> 33 个数学系列（2026-09-06 五刊同日：集合/证明/计算复杂度/应用数学/分析家族；同日第二批：GB/T 13745 全景 16 家族），从公理地基到应用前沿，一个宇宙讲透。
>
...
```
     ...
     讲透分析    群论      高维概率   数值线代    Lean4数学
     (十支家族)  Artin抽代 统计学习理论 信息论
                          代数拓扑           优化
                                            数学建模
                                            博弈论
                          ──────────
...
> 🗺️ **GB/T 13745-2009 全景对齐**（28 个二级学科目录级穷尽，16 新家族同批入册）：[GB-T13745全景.md](GB-T13745全景.md)
> 🧭 **遇到问题找数学工具，第一时间**（问题特征→分支索引+组合模式）：[数学问题路由.md](数学问题路由.md)
...
| 基础 | [`讲透数理逻辑/`](讲透数理逻辑) | **家族总纲**：数理逻辑与数学基础（演绎逻辑/递归论/模型论/数学基础+证明·集合·Lean 就地） | Enderton 等 | ✅ 五章+4支（GB/T 11014） |
| 基础 | [`讲透数学史/`](讲透数学史) | 四线数学史（古代/近代/现代/中国） | Katz 等 | ✅ 五章+4支（GB/T 11011） |
| 基础 | [`讲透离散数学/`](讲透离散数学) | CS 基础视角聚合（关系与序/Boole/递推与生成函数） | Rosen 等 | ✅ 五章+3支（GB/T 11081） |
| 分析 | [`讲透分析/`](讲透分析) | **家族总纲**：十支子目录穷尽分析学（数学分析/实分析/复分析/泛函/调和/数值/变分/动力/PDE/进阶统读线） | Spivak→Tao→Stein 全谱 | ✅ 家族五章（五问第四例·父容器拓扑首例） |
...
| 代数 | [`讲透数论/`](讲透数论) | 整数的显微镜：初等→解析→代数→计算 | Hardy&Wright 等 | ✅ 五章+8支（GB/T 11017） |
| 代数 | [`讲透代数几何/`](讲透代数几何) | 簇→概形→算术几何 | Hartshorne 等 | ✅ 五章+3支（GB/T 11024） |
| 代数 | [`讲透组合数学/`](讲透组合数学) | 计数/极值与 Ramsey/代数组合 | Stanley 等 | ✅ 五章+3支（GB/T 11077） |
| 几何拓扑 🆕 | [`讲透几何学/`](讲透几何学) | 从欧氏到黎曼：变换群纲领十支 | do Carmo 等 | ✅ 五章+10支（GB/T 11027） |
| 几何拓扑 🆕 | [`讲透拓扑学/`](讲透拓扑学) | 点集/同伦/低维/微分+代数拓扑就地 | Munkres 等 | ✅ 五章+10支（GB/T 11031） |
...
| 概率 | [`讲透概率论/`](讲透概率论) | **家族总纲**：GB/T 11064 八支+高维概率就地（承接随机分析） | Durrett 等 | ✅ 五章+8支 |
| 概率 | [`讲透数理统计/`](讲透数理统计) | 抽样到空间统计十二支+SLT 就地 | Casella&Berger 等 | ✅ 五章+12支（GB/T 11067） |
| 概率 | [`讲透应用统计数学/`](讲透应用统计数学) | 可靠性/保险/模拟五支 | Ross 等 | ✅ 五章+5支（GB/T 11071） |
| 应用 | [`讲透应用数学/`](讲透应用数学) | **应用书架方法论总纲**：量纲/渐近/摄动/适定性/稳定性 + SciML 重写现场 | Lin & Segel + Bender-Orszag + Strogatz | ✅ 五章（五问第三例） |
| 应用 | [`讲透数值线代/`](讲透数值线代) | 浮点数下的线代：SVD/条件数/迭代法 | Trefethen & Bau | 📝 2/7 |
| 应用 | [`讲透数学建模/`](讲透数学建模) | 机理→优化→概率统计→数据驱动→AI | 自编（覆盖建模全景） | ✅ 10 篇（8 章+五问X光+来源分类学补篇） |
...
| 应用 | [`讲透运筹学/`](讲透运筹学) | **家族总纲**：排队/库存/决策/搜索/统筹+优化·博弈论·图就地 | Hillier 等 | ✅ 五章+7支（GB/T 11074） |
| 应用 | [`讲透模糊数学/`](讲透模糊数学) | 隶属函数→模糊推理→应用 | Klir&Yuan 等 | ✅ 五章+3支（GB/T 11084） |
...
| 形式化 | [`讲透计算机数学/`](讲透计算机数学) | **家族总纲**：可计算/算法分析/程序验证+Lean·证明·复杂度就地 | Sipser 等 | ✅ 五章+3支（GB/T 11085） |
